...
## 2026-06-07: SPV-2 プロダクト LP をゼロから実装・本番公開（5 commit）

### 概要

SPV-2 LP（`spv.tipsytapstudio.com`）をプレースホルダから本実装へ。前セッションで確定した `apps/spv/SITE_PLAN.md`（IA + 全原稿）を土台に、ネオン計器トーンの 10 セクションを構築。ヒーローの「アニメするブランドロゴ」に最も時間を割き、ユーザーと多数の往復で磨き上げた。`336346c`（初版）→ `ec98c8d`（修正群）→ `b9ee35b`（NTS-2 言及削除）を本番公開。長尺セッション。

### 主要マイルストーン

#### 1. 基盤（spv 専用デザインシステム）
- `apps/spv/src/layouts/Base.astro` 新規。深黒 `#0A0A0A` + ネオンアクセント（シアン/マゼンタ〜パープル/グリーン）のトークン。フォントは **Space Grotesk + JetBrains Mono を self-host**（`@fontsource-variable` 追加）
- **CLAUDE.md「全サブドメイン 1 フォント統一」原則を破棄**（2026-06-07）→「各プロダクトの世界観優先」に改訂。ハブ=Fraunces、SPV-2=計器サンセリフ。シリーズの絆は spacing/レイアウト体系と色階層で担保
- **en 単一化**: ja ページ削除、astro i18n を en のみに、`/`→`/en/` リダイレクト、言語スイッチャ撤去。「日本語が必要かは後で再考」というユーザー判断。i18n キーは parity 維持で ja.json にも残置（未使用）

#### 2. 10 セクション実装（en/index.astro）
Raycast 型（product-as-hero × 縦スクロール）+ KORG トーン。Hero / Demo(YouTube facade) / 5 Modules / Themes / Supported Sites(テキスト+中立アイコン) / Privacy / SP3原則 / Release Notes(v1.0.0〜v1.3.1、releases.json データ化) / Final CTA / Footer。全コピーは `spv_*` i18n キー（約67、ja/en）。コンポーネント新規 7（Header/Footer/InstrumentVisual/ModuleBlock/DemoVideo/SiteGrid/LogoMosaic）

#### 3. ヒーローのアニメするブランドロゴ（最大の作り込み）
SVP-2 ロゴ（icon.svg = **128×128 の 280 タイル角丸モザイク**、ティール→緑→琥珀グラデ）を主役化。`logo-tiles.json` にパースして使用。ユーザーとの往復で段階的に完成:
- 当初: 単一の対角スイープ → 「街のネオンのように緩急・多パターンが欲しい」
- 8 パターン巡回（awaken/diagonal/vertical/horizontal/radial/flicker/pulse/settle）。各タイルに全パターンの位相 `--p0〜--p7` をビルド時に焼き込み、`data-pattern` で方向を切替
- **「パッと切り替わる」snap の根絶**が核心。真因は keyframe の中間フレームが gain 非依存の固定値で、変化量ゼロでも微動が残り方向切替時に飛んでいた → **全フレームを `--neon-gain` でスケール**し、gain=0 で全タイルが完全同一の静止値になるよう修正。これで「変化量ゼロ＝完全静止」→ その静止中に方向切替＝原理的に飛ばない
- 切替方式: ユーザーの「光らせる変化量をほぼゼロに落として『あれ?止まった?』→ 静止 → 次パターンをゼロから立ち上げ」という説明に沿い、`--wave-gain` を CSS transition で 0↔1 補間（JS は目標値 flip のみ＝毎フレーム計算なし。当初の rAF 版は重く headless が固まったため廃止）
- 速度: 各パターン 10〜15秒 + 凪 2.2秒。停止ボタン（極小アイコン）+ 早送り ▶▶（凪経由で次へ）。**5分で凪状態にて自動停止**（画面外は IO で休止、reduced-motion で静止）

#### 4. YouTube facade
クリックまで iframe を入れない（ネットワークゼロ訴求と両立）。サムネは YouTube hqdefault（480×360、この動画は maxres/sd が無く hqが最大）を self-host、再生で youtube-nocookie 埋込。途中 Store スクショ(1280×800)を試したが「動画に無い画」なので YouTube サムネに戻した。Astro の scoped CSS が動的生成 iframe に効かず左上に小さく出るバグ→ iframe を inline サイズ指定で修正

#### 5. ヘッダー / ファビコン / はみ出し修正（ec98c8d, b9ee35b）
- ヘッダー: 戻りリンクと SPV-2 が被る → 戻りリンクは**フッターに一本化して削除**（多くの人はスタジオに行かない）。ブランドを「[モザイクロゴ] SPV-2」形式に
- ファビコン: プレースホルダ 'SP' 文字 → 本物の SPV-2 モザイク（icon.svg）。ヘッダーロゴと同一マーク
- Five meters に IntersectionObserver で「視界に入ったら動く」（画面外停止）
- **横スクロール除去**: スキップリンク `left:-9999px` が scroll 幅を広げていた → clip 方式に
- **フッター下の巨大空白(~757px)を解消**: ヒーローロゴのグロー（`::before inset:-8%`）が positioned 親を持たず、スキップリンク対策で `body{position:relative}` にしていたため body 基準（108%）まで伸びていた → `.logo-mosaic{position:relative}` でグローをロゴ内に収容。CSS の containing-block 連鎖が原因の地雷
- ヒーローの **NTS-2 言及（hero aside）を削除**（名指し不要）

### プロセス / 学び

- **ヒーローロゴの snap 問題で何度も往復**したが、根本原因（keyframe 固定値 → gain スケール化）を突き止めて解決。ユーザーの「凪にクロスフェード」「変化量をゼロから」という言語化が解決の鍵になった
- **headless プレビューの限界が頻発**: vw:0 / scrollTo no-op / IO 不発火 / screenshot タイムアウト / scrollHeight 不正確。視覚・スクロール挙動は computed style と本番ビルド(`npm run preview`)で間接検証し、最終判断はユーザー実機に委ねた
- **Astro dev の CSS キャッシュが Base.astro 変更を拾わない**ことがあり、`.astro`/`.vite` キャッシュ削除 + サーバ再起動で対処。確実な検証は dist（preview）で実施
- ユーザー指摘の「左右非対称」「フッター下の空白」はどちらも**実在のバグ**（グロー由来）で、体感は正しかった

### 配信中 URL（本番反映、`b9ee35b`）

- https://spv.tipsytapstudio.com/en/ — SPV-2 LP 初版（`/` `/ja/` は `/en/` にリダイレクト）
- ※ ハブからの導線は未設置。「誰も来ない」前提で作りかけを公開

### 残宿題（次セッションへ）

- **Five meters の計器 SVG 作り込み**（次の本命）: オシロ/スペアナ/クロマを**プロダクト準拠**で描く。`~/Desktop/spv/sidepanel.js` の実描画コード参照（クロマは前回抽出済みのピアノロール形状）。虚偽広告にならない範囲で単純化。アニメは後、まず静止 SVG 優先
- **themes 着せ替えデモ**（S4）: 上記 SVG を使い、40+テーマ（10内蔵 + 41 Strudel派生、theme.js から抽出済み）を 1テーマ=1四角でデフォルメ表示 or 簡易着せ替え
- **Where it works 詳細表**（S5）: URL 判定パターン（manifest host_permissions）まで含む表。「なぜ動かないか」が分かる正確な資料
- 背景タイル演出（ロゴと同じタイル primitive がネオンで流れる）
- ハブ Works の SPV-2 カードから LP への導線確認 / LP 全体の最終レビュー
- ロゴアニメの Lab プロダクト化構想（SITE_PLAN 末尾に記録済み）
