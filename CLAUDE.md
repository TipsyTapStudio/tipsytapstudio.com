...
| ハブ＝Brew 背景 (3 ビール循環) + 上層モノトーン UI、プロダクト LP＝アクセント色解放 | 親子関係を「色の階層」で表現。ハブの色は Brew 背景レイヤー専用、UI 層は引き続きモノトーン (2026-05-28 更新) |
| タイポグラフィは各プロダクトの世界観優先 | ~~1 ファミリー全サブドメイン統一~~ を破棄 (2026-06-07)。ハブは Fraunces、SPV-2 等プロダクト LP は固有フォント可 (例: SPV-2 = Space Grotesk + JetBrains Mono の計器トーン)。シリーズの絆は spacing/レイアウト体系と色階層で担保し、フォントはプロダクト個性に解放 |
